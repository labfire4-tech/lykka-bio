...
export default function CustomizePage() {
  ...
  useEffect(() => {
    ...
    if (u) {
      ...
      setUser(parsed);
      // Try loading profile from API first
      ...
      // No user logged in — load from localStorage if exists
      const saved = localStorage.getItem("curzed-profile");
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white flex">
      {/* Sidebar */}
      <aside className="w-[260px] border-r border-white/[0.06] p-4 hidden md:flex flex-col shrink-0 bg-[#080808]">
        <div className="mb-6 px-2">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-black">CURZED</h2>
          </div>
        </div>

        <div className="relative mb-5">
          <Search size="14" className="absolute left-3 top-1/2 -translate-y-1/2 text-white/30" />
          <input type="text" placeholder="Search features..." className="w-full pl-9 pr-12 py-2.5 bg-white/[0.03] border border-white/[0.06] rounded-xl text-xs placeholder:text-white/25 focus:outline-none focus:border-white/20 transition-colors" />
        ...
        <nav className="space-y-1 relative">
          {navItems.map((item) => (
            <button key={item.id} onClick={() => setActiveTab(item.id)} className={"relative w-full flex items-center gap-3 px-4 py-2.5 rounded-full text-sm font-medium transition-all " + (activeTab === item.id ? "text-white" : "text-white/50 hover:text-white/80 hover:bg-white/[0.03]")}>
              {activeTab === item.id && (
                <motion.div layoutId="nav-active" className="absolute inset-0 rounded-full bg-white/[0.08]" transition={{ type: "spring", stiffness: 300, damping: 25 }} />
              )}
          ...
          <Link href="/templates" className="w-full flex items-center gap-3 px-4 py-2.5 rounded-full text-sm font-medium text-white/50 hover:text-white/80 hover:bg-white/[0.03] transition-all">
            <Sparkles size="16" /><span className="flex-1 text-left">Templates</span><span className="text-[9px] bg-white/10 px-1.5 py-0.5 rounded-full font-bold">NEW</span>
          </Link>
        </nav>

        <div className="mt-auto space-y-2 pt-4">
          <Link href="/help" className="flex items-center gap-3 px-4 py-2.5 rounded-full bg-white/[0.03] border border-white/[0.06] text-white/60 text-sm font-medium hover:bg-white/[0.06] transition-all">
            <HelpCircle size="16" /> Help Center
          </Link>
          <Link href={user ? `/${user.username}` : "/"} className="flex items-center gap-3 px-4 py-2.5 rounded-full bg-white/[0.05] text-white text-sm font-medium hover:bg-white/[0.08] transition-all">
            <ExternalLink size="16" /> My Page
          </Link>
          <button onClick={saveProfile} disabled={saving} className="w-full flex items-center gap-3 px-4 py-2.5 rounded-full bg-white text-black text-sm font-medium hover:bg-white/90 transition-all disabled:opacity-50">
      ...
      <div className="md:hidden fixed top-0 left-0 right-0 z-20 bg-[#080808]/95 backdrop-blur border-b border-white/[0.06]">
        <div className="p-3"><h2 className="text-base font-black">CURZED</h2></div>
        <div className="flex gap-2 px-4 pb-3 overflow-x-auto">
          {navItems.map((item) => (
            <button key={item.id} onClick={() => setActiveTab(item.id)} className={"px-4 py-2 rounded-full text-xs font-bold whitespace-nowrap transition-all " + (activeTab === item.id ? "bg-white/[0.1] text-white" : "bg-white/5 text-white/50")}>{item.label}</button>
          ))}
        </div>
      </div>
      ...
      <main className="flex-1 overflow-y-auto pt-20 md:pt-0">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="relative rounded-2xl overflow-hidden border border-white/[0.08]">
            ...
            <div className="relative p-5 flex flex-col md:flex-row items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                ...
                <div><h3 className="font-bold text-sm">Want exclusive features?</h3><p className="text-xs text-white/40 mt-0.5">Unlock more with <span className="text-white font-bold">Premium</span></p></div>
              </div>
              <div className="flex gap-2">
                <Link href="/premium" className="px-5 py-2.5 bg-white text-black text-xs font-bold rounded-full transition-colors hover:bg-white/90">Upgrade Now</Link>
                <Link href="/premium" className="px-5 py-2.5 bg-white/[0.05] hover:bg-white/[0.08] text-white text-xs font-bold rounded-full transition-colors">Learn More</Link>
              </div>
            </div>
          </motion.div>
            ...
            >
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="text" value={p.description} onChange={(e) => update({ description: e.target.value })} className="w-full pl-11 pr-4 py-3 bg-white/[0.03] border border-white/[0.06] rounded-xl text-sm focus:outline-none focus:border-white/20 transition-colors" />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <div className="flex items-center bg-white/[0.03] border border-white/[0.06] rounded-xl p-1.5">
                    ...
                    <div className="flex items-center gap-1 pr-2">
                      <button onClick={() => update({ discordPresence: !p.discordPresence })} className={"w-9 h-5 rounded-full relative transition-colors " + (p.discordPresence ? "bg-white" : "bg-white/15")}>
                        <motion.div className="absolute top-0.5 w-4 h-4 bg-black rounded-full shadow" animate={{ left: p.discordPresence ? "22px" : "2px" }} transition={{ type: "spring", stiffness: 500, damping: 30 }} />
                      </button>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <select value={p.location} onChange={(e) => update({ location: e.target.value })} className="w-full pl-11 pr-4 py-3 bg-white/[0.03] border border-white/[0.06] rounded-xl text-sm text-white/70 focus:outline-none focus:border-white/20 appearance-none transition-colors">
                ...
                {[
                  ...
                  <div key={slider.label}>
                    ...
                    <div className="relative h-8 flex items-center group">
                      ...
                      <motion.div className="absolute w-4 h-4 bg-white rounded-full shadow pointer-events-none" animate={{ left: `calc(${slider.value}% - 8px)` }} transition={{ type: "spring", stiffness: 500, damping: 30 }} />
                    </div>
              ...
              <div>
                ...
                <div className="relative">
                  <select value={p.backgroundEffect} onChange={(e) => update({ backgroundEffect: e.target.value })} className="w-full px-4 py-3 bg-white/[0.03] border border-white/[0.06] rounded-xl text-sm text-white/70 focus:outline-none focus:border-white/20 appearance-none transition-colors">
                ...
                <div className="flex gap-3">
                  {([
                    ...
                    <motion.button
                      ...
                      className={"flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-bold transition-all " + (toggle.active ? "bg-white text-black" : "bg-white/[0.03] text-white/40 hover:bg-white/[0.06] border border-white/[0.06]")}
                    >
        ...
        <div className="md:hidden fixed bottom-0 left-0 right-0 p-4 bg-[#050505]/90 backdrop-blur border-t border-white/[0.06] z-20">
          <button onClick={saveProfile} disabled={saving} className="w-full py-3.5 bg-white text-black hover:bg-white/90 disabled:opacity-50 font-bold rounded-full transition-all">{saving ? "Saving..." : "Save Changes"}</button>
        </div>
              ...
              <div className="relative">
                ...
                <div className="w-[260px] bg-[#0a0a0a] rounded-[2.5rem] border-4 border-[#1a1a1a] shadow-2xl overflow-hidden">
                  ...
                  <div className="px-5 pt-4 pb-6 text-center relative">
                    ...
                    <div className="relative">
                      <div className="w-20 h-20 rounded-full mx-auto mb-3 border-2 border-white/20 overflow-hidden bg-white/[0.05] flex items-center justify-center text-3xl" style={{ filter: `blur(${p.profileBlur / 10}px)`, opacity: p.profileOpacity / 100, backgroundImage: p.profileImage ? `url(${p.profileImage})` : undefined, backgroundSize: "cover", backgroundPosition: "center" }}>{!p.profileImage && "◆"}</div>
                      <h3 className={"font-black text-sm " + (p.glowUsername ? "text-white" : "text-white/80")} style={p.glowUsername ? { textShadow: "0 0 10px rgba(255,255,255,0.3)" } : {}}>@{user?.username || "username"}</h3>
                  ...
                  <div className="px-4 space-y-2 pb-6">
                    {[{ name: "Twitter / X", color: "#ffffff" }, { name: "Instagram", color: "#ffffff" }, { name: "YouTube", color: "#ffffff" }].map((link) => (
                      <div key={link.name} className={"py-3 rounded-xl text-center text-xs font-bold transition-all " + (p.glowSocials ? "" : "bg-white/[0.05] text-white")} style={p.glowSocials ? { backgroundColor: `rgba(255,255,255,0.08)`, color: "#fff", border: `1px solid rgba(255,255,255,0.1)`, boxShadow: `0 0 12px rgba(255,255,255,0.1)` } : {}}>{link.name}</div>
                    ))}
                  </div>
                </div>
              </div>
        ...
        {!showPreview && <button onClick={() => setShowPreview(true)} className="hidden lg:flex fixed right-6 bottom-6 z-20 px-4 py-2.5 bg-white text-black hover:bg-white/90 text-xs font-bold rounded-full shadow-lg transition-all">Show Preview</button>}
      </main>
    </div>
  );
}
