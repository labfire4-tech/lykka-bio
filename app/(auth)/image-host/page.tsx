...
import { Upload, Image as ImageIcon, Trash2, ExternalLink, Copy, Check, FileImage, Film } from "lucide-react";
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-sm text-white/50">Upload and manage your images</p>
          </div>
        ...
        <div
          ...
          className={`border-2 border-dashed rounded-2xl p-12 text-center mb-8 transition-all ${
            dragOver ? "border-purple-500/50 bg-purple-500/5" : "border-white/10 hover:border-purple-500/30"
          }`}
        >
          ...
          <p className="text-sm font-medium text-white/70">Drag & drop files here</p>
          <p className="text-xs text-white/40 mt-1">or click to browse — PNG, JPG, GIF, MP4 up to 25MB</p>
        </div>

        {/* View toggle */}
        <div className="flex items-center justify-between mb-4">
          <p className="text-xs text-white/40">{images.length} files</p>
          <div className="flex gap-1 p-1 rounded-lg bg-white/5 border border-white/10">
            <button
              onClick={() => setView("grid")}
              className={`px-3 py-1 rounded text-xs font-medium transition-all ${view === "grid" ? "bg-purple-600 text-white" : "text-white/50"}`}
            >
              Grid
            </button>
            <button
              onClick={() => setView("list")}
              className={`px-3 py-1 rounded text-xs font-medium transition-all ${view === "list" ? "bg-purple-600 text-white" : "text-white/50"}`}
            >
              List
            </button>
          </div>
              ...
              {images.map((img, i) => (
                ...
                >
                  <div className="aspect-square bg-white/5 relative overflow-hidden">
                    {img.url ? (
                      ...
                      <div className="w-full h-full flex items-center justify-center">
                        {img.type === "video" ? <Film size={28} className="text-white/30" /> : <ImageIcon size={28} className="text-white/30" />}
                      </div>
                    )}
                    {/* Hover actions */}
                    <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                      ...
                      >
                        {copied === img.id ? <Check size={14} className="text-green-400" /> : <Copy size={14} />}
                      </button>
                  ...
                  <div className="p-3">
                    ...
                    <p className="text-[10px] text-white/40">{img.size}</p>
                  </div>
                </motion.div>
              ))}
            </motion.div>
            ...
            >
              {images.map((img, i) => (
                <motion.div
                  ...
                  transition={{ delay: i * 0.05 }}
                  className="card-hover p-3 flex items-center gap-4"
                >
                  <div className="w-12 h-12 rounded-lg bg-white/5 overflow-hidden shrink-0">
                    {img.url ? (
                      ...
                      <div className="w-full h-full flex items-center justify-center">
                        {img.type === "video" ? <Film size={18} className="text-white/30" /> : <FileImage size={18} className="text-white/30" />}
                      </div>
                    )}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-medium truncate">{img.name}</div>
                    <div className="text-xs text-white/40">{img.size} • {img.date}</div>
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    >
                      {copied === img.id ? <Check size={14} className="text-green-400" /> : <Copy size={14} className="text-white/60" />}
                    </button>
