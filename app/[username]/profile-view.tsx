...
import { Eye, Copy, Check, Volume2, VolumeX, Play, Pause, SkipBack, SkipForward, Music, ChevronRight } from "lucide-react";
...
function SocialIcon({ name, color }: { name: string; color: string }) {
  const icons: Record<string, React.ReactNode> = {
    ...
    spotify: <svg viewBox="0 0 24 24" width="22" height="22" fill="currentColor"><path d="M12 0C5.4 0 0 5.4 0 12s5.4 12 12 12 12-5.4 12-12S18.66 0 12 0zm5.521 17.34c-.24.359-.66.48-1.021.24-2.82-1.74-6.36-2.101-10.561-1.141-.418.122-.779-.179-.899-.539-.12-.421.18-.78.54-.9 4.56-1.021 8.52-.6 11.64 1.32.42.18.479.659.301 1.02zm1.44-3.3c-.301.42-.841.6-1.262.3-3.239-1.98-8.159-2.58-11.939-1.38-.479.12-1.02-.12-1.14-.6-.12-.48.12-1.021.6-1.141C9.6 9.9 15 10.561 18.72 12.84c.361.181.54.78.241 1.2zm.12-3.36C15.24 8.4 8.82 8.16 5.16 9.301c-.6.179-1.2-.181-1.38-.721-.18-.601.18-1.2.72-1.381 4.26-1.26 11.28-1.02 15.721 1.621.539.3.719 1.02.419 1.56-.299.421-1.02.599-1.559.3z" /></svg>,
    twitch: <svg viewBox="0 0 24 24" width="22" height="22" fill="currentColor"><path d="M11.571 4.714h1.715v5.143H11.57zm4.715 0H18v5.143h-1.714zM6 0L1.714 4.286v15.428h5.143V24l4.286-4.286h3.428L22.286 12V0zm14.571 11.143l-3.428 3.428h-3.429l-3 3v-3H6.857V1.714h13.714Z" /></svg>,
...
export default function ProfileView() {
  ...
  return (
    <div className="min-h-screen relative overflow-hidden" style={{ background: theme.backgroundColor || "#050505" }}>
      {/* Background video */}
      {profile.bgVideo && (
        <video ref={videoRef} loop playsInline muted className="absolute inset-0 w-full h-full object-cover" style={{ filter: "brightness(0.35) contrast(1.1)" }}>
      ...
      {profile.bgImage && !profile.bgVideo && (
        <div className="absolute inset-0">
          <img src={profile.bgImage} alt="" className="w-full h-full object-cover" style={{ filter: "brightness(0.35)" }} />
        </div>
      )}
      ...
      <div className="absolute inset-0 pointer-events-none">
        <motion.div className="absolute top-1/4 -left-32 w-96 h-96 rounded-full blur-[120px]" style={{ backgroundColor: `${theme.accentColor}12` }} animate={{ scale: [1, 1.2, 1], opacity: [0.3, 0.5, 0.3] }} transition={{ duration: 8, repeat: Infinity }} />
        <motion.div className="absolute bottom-1/4 -right-32 w-80 h-80 rounded-full blur-[100px]" style={{ backgroundColor: `${theme.accentColor}08` }} animate={{ scale: [1, 1.3, 1], opacity: [0.2, 0.4, 0.2] }} transition={{ duration: 10, repeat: Infinity }} />
      </div>
      ...
      <AnimatePresence>
        {!entered && (
          <motion.div
            ...
            onClick={handleEnter}
            className="absolute inset-0 z-[100] bg-black/80 backdrop-blur-xl flex items-center justify-center cursor-pointer"
          >
            <motion.div
            ...
            >
              <p className="text-sm tracking-[0.3em] uppercase text-white/80 font-medium">
                [ click to enter ]
              </p>
            </motion.div>
          </motion.div>
      ...
      <AnimatePresence>
        {entered && (
          <motion.button
            ...
            onClick={toggleMute}
            className="fixed top-6 left-6 w-11 h-11 rounded-xl bg-black/60 backdrop-blur-md border border-white/10 flex items-center justify-center z-20 hover:bg-black/80 hover:scale-105 transition-all"
          >
      ...
      {/* ===== Main Content ===== */}
      <div className={"min-h-screen flex items-center justify-center p-6 relative z-10 transition-all duration-1000 " + (entered ? "opacity-100" : "opacity-0")}>
        <motion.div
        ...
        >
          {/* Profile card */}
          <div className="rounded-3xl p-8 backdrop-blur-2xl border" style={{ backgroundColor: "rgba(0,0,0,0.65)", borderColor: `${theme.accentColor}25`, boxShadow: `0 25px 50px -12px rgba(0,0,0,0.5), inset 0 0 60px ${theme.accentColor}08` }}>

            {/* Avatar with multi-layer glow rings */}
            <motion.div
            ...
            >
              <motion.div className="absolute -inset-3 rounded-full" style={{ background: `conic-gradient(from 0deg, ${theme.accentColor}, transparent 60%, ${theme.accentColor})` }} animate={{ rotate: 360 }} transition={{ duration: 4, repeat: Infinity, ease: "linear" }} />
              <motion.div className="absolute -inset-1 rounded-full" style={{ background: theme.accentColor, opacity: 0.2, filter: "blur(8px)" }} animate={{ scale: [1, 1.15, 1], opacity: [0.15, 0.3, 0.15] }} transition={{ duration: 2, repeat: Infinity }} />
            ...
            >
              {/* Sparkle particles */}
              <div className="absolute inset-0 pointer-events-none" style={{ opacity: 0.4, backgroundImage: "radial-gradient(circle at 10% 20%, white 1px, transparent 1.5px), radial-gradient(circle at 90% 80%, white 1px, transparent 1.5px), radial-gradient(circle at 80% 10%, white 1px, transparent 1.5px), radial-gradient(circle at 20% 90%, white 1px, transparent 1.5px), radial-gradient(circle at 50% 0%, white 1px, transparent 1.5px)", animation: "flicker 4s infinite alternate" }} />
              <h1 className="text-3xl font-bold tracking-tight relative z-10" style={{ color: theme.textColor || "#fff", textShadow: `0 0 20px ${theme.accentColor}40` }}>
                {profile.displayName}
              </h1>
            </motion.div>

            {/* Bio */}
            <motion.div initial={{ opacity: 0, y: 15 }} animate={entered ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5, delay: 0.35 }} className="text-center mb-5">
              <p className="text-sm leading-relaxed" style={{ color: `${theme.textColor}80` }}>{profile.bio || "Digital creator"}</p>
            </motion.div>

            {/* Social icons row */}
            <motion.div initial={{ opacity: 0, y: 15 }} animate={entered ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5, delay: 0.45 }} className="flex items-center justify-center gap-5 mb-6">
              {profile.socialLinks.map((link, idx) => {
                ...
                return (
                  <motion.a
                    ...
                    animate={entered ? { opacity: 0.9, scale: 1 } : {}}
            ...
            {/* Copy link + views row */}
            <motion.div initial={{ opacity: 0 }} animate={entered ? { opacity: 1 } : {}} transition={{ delay: 0.7 }} className="flex items-center justify-center gap-3">
              <button onClick={handleCopyLink} className="flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-medium transition-all hover:scale-105" style={{ backgroundColor: `${theme.accentColor}15`, color: theme.textColor || "#fff", border: `1px solid ${theme.accentColor}30` }}>
                {copied ? <><Check size={12} /> Copied</> : <><Copy size={12} /> Copy</>}
              </button>
              <div className="flex items-center gap-2 px-3 py-1.5 rounded-full text-xs bg-black/40 backdrop-blur border border-white/5">
                <Eye size={12} className="text-white/40" />
                <span className="text-white/50"><span className="font-semibold text-white/70">{animatedViews.toLocaleString()}</span></span>
              </div>
            </motion.div>
          </div>

          {/* Footer */}
          <motion.div initial={{ opacity: 0 }} animate={entered ? { opacity: 1 } : {}} transition={{ delay: 1 }} className="text-center mt-6">
            <a href="/" className="text-xs text-white/25 hover:text-white/50 transition-colors">Powered by <span className="font-bold">LYKKA</span><span style={{ color: theme.accentColor }}>.</span>bio</a>
          </motion.div>
        </motion.div>
      </div>
      ...
      <AnimatePresence>
        {entered && profile.bgVideo && (
          <motion.div
            ...
            className="fixed bottom-6 left-1/2 -translate-x-1/2 z-20 w-[450px] max-w-[90%]"
          >
            <div className="flex items-center gap-3 p-3 rounded-2xl bg-black/60 backdrop-blur-md border border-white/[0.06]">
              {/* Icon */}
              <div className="w-12 h-12 rounded-xl bg-white/5 flex items-center justify-center shrink-0 border border-white/[0.06]">
