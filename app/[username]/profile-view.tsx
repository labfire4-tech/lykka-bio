...
import { Eye, Copy, Check, ExternalLink, MapPin, Sparkles } from "lucide-react";
...
function SocialIcon({ name, color }: { name: string; color: string }) {
  const icons: Record<string, React.ReactNode> = {
    ...
    steam: (
      <svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor">
        <path d="M11.979 0C5.678 0 .511 4.86.022 11.037l6.432 2.658c.545-.371 1.203-.59 1.912-.59.063 0 .125.004.188.006l2.861-4.142V8.91c0-2.495 2.028-4.524 4.524-4.524 2.494 0 4.524 2.031 4.524 4.527s-2.03 4.526-4.524 4.526h-.105l-4.076 2.911c0 .052.004.105.004.159 0 1.875-1.523 3.398-3.398 3.398-1.639 0-3.012-1.165-3.331-2.717L.436 15.27C1.862 20.307 6.486 24 11.979 24c6.627 0 11.999-5.373 11.999-12S18.605 0 11.979 0zM7.54 18.21l-1.473-.61c.262.543.714.999 1.314 1.25 1.297.539 2.793-.076 3.332-1.375.263-.63.264-1.331.005-1.963s-.753-1.121-1.382-1.383c-.625-.26-1.314-.255-1.913-.009l1.523.63c.956.398 1.409 1.492 1.011 2.448-.397.956-1.491 1.409-2.447 1.012zm13.616-9.299c0-1.661-1.35-3.013-3.012-3.013-1.661 0-3.012 1.352-3.012 3.013 0 1.661 1.351 3.012 3.012 3.012 1.662 0 3.012-1.351 3.012-3.012zm-5.276-.005c0-1.252 1.013-2.266 2.265-2.266 1.253 0 2.266 1.014 2.266 2.266 0 1.251-1.013 2.266-2.266 2.266-1.252 0-2.265-1.015-2.265-2.266z" />
      </svg>
    ),
    default: (
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-black text-white px-6">
        ...
        >
          ...
          <a href="/customize" className="btn-primary inline-flex">
            Create Your Profile
          </a>
        </motion.div>
      </div>
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            <p className="text-sm leading-relaxed" style={{ color: `${theme.textColor}80` }}>
            ...
            {profile.socialLinks.map((link, idx) => {
              ...
              return (
                <motion.a
                  ...
                  whileTap={{ scale: 0.98 }}
                  className="group flex items-center gap-4 w-full px-5 py-3.5 rounded-2xl backdrop-blur-md transition-all duration-300"
                  style={{
        ...
        >
          ...
          >
            Powered by <span className="font-bold">LYKKA</span>
            <span style={{ color: theme.accentColor }}>.</span>bio
          </a>
        </motion.div>
      </motion.div>
    </div>
